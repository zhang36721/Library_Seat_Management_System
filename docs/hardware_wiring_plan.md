...
|------|---------------|------------|----------|
| 主控端模块 | `0x0000` | USART1 PA9/PA10 | 发送给座位端使用 `0x301E` |
| 座位端模块 | `0x301E` | USART1 PA9/PA10 | 发送给主控端使用 `0x0000` |
...
地址低字节在前。例如主控发给座位端 `0x301E` 时地址为 `1E 30`；座位端发给主控 `0x0000` 时地址为 `00 00`。
...
| ZigBee | `FF 82 00 FF` | USART1 向 `0x301E` 发送二进制 `01 seq` PING |
