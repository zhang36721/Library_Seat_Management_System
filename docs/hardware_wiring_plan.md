...
本文档记录当前 STM32F103C8T6 主控和 seat_node_stm32 座位端的接线规划。v0.7.1/v0.8 只做本地硬件初始化、自检和 ZigBee 串口透传验证，不接后端、不接前端、不做完整业务闭环。
...
| ESP32S3 | USART3 | PB10/PB11 | v0.7 只发送测试字符串 |

## 6. seat_node_stm32 v0.8 接线规划
