...
| 串口 | STM32 引脚 | 职责 | 当前状态 |
|------|------------|------|----------|
| USART1 | PA9 TX / PA10 RX | CC2530 ZigBee 串口点对点通信，38400 | v0.7.1 已支持 TX/RX 测试，不代表无线链路已通 |
...
当前座位端不再按 3 路红外/低电平输入处理。最新规划如下：

| 座位 | 传感器/输入 | STM32 引脚 | 模式 | 有效判断 | 备注 |
|------|------------|------------|------|----------|------|
| 座位 1 | HX711 + 压力/重力传感器 + 红外 | PA4 = HX711 DT，PA5 = HX711 SCK，PB9 = 红外输入 | PA4 输入上拉，PA5 输出，PB9 输入下拉 | 重量 `> 50g` 且 PB9 红外触发时判定 `OCCUPIED` | VCC 接 3.3V，GND 共地；红外有效电平需实机确认 |
| 座位 2 | GPIO 高电平模拟重力 | PB8 | GPIO 输入下拉 | 高电平 = `OCCUPIED`，低电平 = `FREE` | 不使用红外 |
| 座位 3 | GPIO 高电平模拟重力 | PB7 | GPIO 输入下拉 | 高电平 = `OCCUPIED`，低电平 = `FREE` | 不使用红外 |

HX711 模块接线建议：压力传感器红线接 E+，黑线接 E-，白线接 A-，绿线接 A+；HX711 VCC 接 STM32 3.3V，GND 接 STM32 GND，DT 接 PA4，SCK 接 PA5。座位 1 红外模块输出接 PB9。若实物接线不同，以实物为准同步修改 `kt_config.h` 中的 `SEAT1_HX711_*` / `SEAT1_IR_*` 宏。

座位端 debug 命令保持不变：`FF 30 00 FF` 打印座位 1 HX711 原始值/差值、PB9 红外原始电平和座位 2/3 GPIO 原始电平；`FF 31 00 FF` 打印 3 个座位 `FREE/OCCUPIED/UNKNOWN` 判断；`FF 80 00 FF` 通过 ZigBee 地址帧发送当前二进制座位状态 `10 S1 S2 S3`。

HX711 克重输出需要实机标定：空座时发送 `FF 32 00 FF` 去皮；放上已知重量后发送 `FF 33 NN FF` 标定，`NN` 表示 `NN*10g`，例如 50g 发送 `FF 33 05 FF`，500g 发送 `FF 33 32 FF`；之后发送 `FF 34 00 FF` 可打印估算重量，单位 g。标定参数保存到座位端 Flash `0x0800FC00`，断电重启后会自动加载。

当前固件内置一组实机默认标定参数，作为 Flash 被擦除或校验失败时的 fallback：`offset=-250747`，`scale=40667 counts/g x100`。实机日志验证：230g 标定后读数约 229~230g，约 111g 样品读数约 110~111g，约 33g 样品读数约 32~33g。若后续重新安装压力传感器或机械结构改变，应重新执行 `FF 32` / `FF 33` 标定并更新默认宏。

座位端主循环每 `200ms` 轮询一次 3 个座位，连续 3 次采样一致后才确认状态变化。上电后发送一次座位状态，之后只在座位状态稳定变化时打印一次来人/离开变化日志并上报 ZigBee；状态不变时不刷屏。发送 `FF 31 00 FF` 时会主动打印 3 个座位的当前判断结果。
...
| 座位检测 1 | PA4/PA5 + PB9 | HX711 压力/重力 + 红外共同判断 | 已初始化 | 重量 `> 50g` 且 PB9 红外触发时为占用 |
| 座位检测 2 | PB8 | 高电平模拟重力输入 | 已初始化 | 高电平为占用，不使用红外 |
| 座位检测 3 | PB7 | 高电平模拟重力输入 | 已初始化 | 高电平为占用，不使用红外 |

seat_node_stm32 当前座位 2/3 GPIO active level 为 `GPIO_PIN_SET`，即高电平判断为 `OCCUPIED`。座位 1 红外当前按 `SEAT1_IR_ACTIVE_LEVEL = GPIO_PIN_SET` 处理；若实物红外为低电平有效，修改该宏后复测。

### CC2530 ZigBee 点对点帧格式

当前两个 CC2530 模块不是纯透明串口模式，STM32 发送到 ZigBee 模块时需要带目标短地址。USART1 仍为 ZigBee 专用串口，USART2 继续保留为 debug。

| 角色 | ZigBee 短地址 | STM32 串口 | 目标地址 |
|------|---------------|------------|----------|
| 主控端模块 | `0x301E` | USART1 PA9/PA10 | 发送给座位端使用 `0x0000` |
| 座位端模块 | `0x0000` | USART1 PA9/PA10 | 发送给主控端使用 `0x301E` |
...
地址低字节在前。模块对换后，主控发给座位端 `0x0000` 时地址为 `00 00`；座位端发给主控 `0x301E` 时地址为 `1E 30`。
...
座位端上电发送一次座位状态，之后只在稳定变化时上报，实际帧为：

```text
FA 00 00 13 53 45 41 54 2C 53 31 3D 30 2C 53 32 3D 31 2C 53 33 3D 30 F5
```

主控端只按完整 `FA ... F5` 二进制帧解析，不再兼容 ASCII 文本透传。主控收到 `10 S1 S2 S3` 后更新 Web 使用的内部座位状态，并打印：

```text
[ZIGBEE] RX SEAT: S1=0 S2=1 S3=0 FREE=2
```
...
| ZigBee | `FF 82 00 FF` | USART1 向座位端 `0x0000` 发送二进制 `01 seq` PING |
| ZigBee | `FF 84 00 FF` | 打印 ZigBee 二进制帧统计 |
| ZigBee | `FF 85 00 FF` | 清空 ZigBee 二进制帧统计 |
| ZigBee | `FF 86 00 FF` | 发送 ZigBee 二进制 PING |
...
| HX711 | `FF 32 00 FF` | 空座去皮 |
| HX711 | `FF 33 NN FF` | 用 `NN*10g` 已知重量标定，并保存到 Flash |
| HX711 | `FF 34 00 FF` | 打印座位 1 估算克重 |
| ZigBee | `FF 80 00 FF` | 通过 USART1 发送当前二进制座位状态 `10 S1 S2 S3` |
| ZigBee | `FF 81 00 FF` | 发送二进制 `02 seq` PONG 测试帧 |
