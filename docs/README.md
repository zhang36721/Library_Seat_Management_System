# 项目文档

本目录包含项目的所有技术文档和设计说明。

## 文档列表

| 文件 | 说明 |
|------|------|
| `01_project_overview.md` | 项目概述、需求和整体架构 |
| `02_hardware_design.md` | 硬件设计说明、原理图和PCB设计 |
| `03_communication_protocol.md` | 通信协议定义和数据格式 |
| `04_software_design.md` | 软件设计、数据库和API设计 |
| `05_test_record.md` | 测试记录和测试报告 |
| `hardware_pin_map.md` | STM32 主控硬件引脚资源表 |
| `hardware_wiring_plan.md` | 主控和座位端接线布局规划 |
| `hardware_module_test_report.md` | 主控外设、座位端、v0.8.2 本地刷卡、v0.8.3 按键菜单、v0.8.4 菜单功能闭环和 v0.8.5 自动通行测试报告 |

## 文档维护

- 文档应随项目进展及时更新
- 重大设计变更需要更新相关文档
- 测试记录应定期整理归档
