...
## seat_node_stm32 座位端 v1.3 更新

| 模块 | 功能 | 引脚 | 模式 | 有效电平/判断 | 默认状态 | 是否已初始化 | 是否已验证 | 备注 |
|------|------|------|------|---------------|----------|--------------|------------|------|
| 座位端座位 1 | HX711 压力/重力 + 红外共同判断 | PA4 = DT，PA5 = SCK，PB9 = 红外输入 | PA4 输入上拉，PA5 输出推挽，PB9 输入下拉 | 重量 `> 50g` 且 PB9 触发为 `OCCUPIED` | SCK 低电平，PB9 下拉，HX711 未 ready 时 `UNKNOWN` | 是 | 待实机压测 | HX711 VCC 接 3.3V，GND 共地；红外有效电平当前按高电平有效；状态变化时打印一次 |
| 座位端座位 2 | 高电平模拟重力输入 | PB8 | GPIO 输入下拉 | 高电平为 `OCCUPIED`，低电平为 `FREE` | 下拉，默认 `FREE` | 是 | 待实机验证 | 不使用红外 |
| 座位端座位 3 | 高电平模拟重力输入 | PB7 | GPIO 输入下拉 | 高电平为 `OCCUPIED`，低电平为 `FREE` | 下拉，默认 `FREE` | 是 | 待实机验证 | 不使用红外 |
| 座位端 Flash | HX711 标定参数 | `0x0800FC00` | Flash 最后一页 | magic + offset + scale + checksum | 无合法数据时使用固件默认参数 | 是 | 待掉电验证 | Keil IROM 已预留最后 1KB；默认 `offset=-250747`，`scale=40667 counts/g x100` |
| ZigBee 主控端 | CC2530 协调器 | USART1 PA9/PA10 | UART 38400 8N1 | 短地址 `0x0000` | 目标座位端 `0x301E` | 是 | 待点对点实测 | 发送帧 `FA 1E 30 LEN DATA F5`，USART2 不参与 |
| ZigBee 座位端 | CC2530 终端节点 | USART1 PA9/PA10 | UART 38400 8N1 | 短地址 `0x301E` | 目标主控端 `0x0000` | 是 | 待点对点实测 | 发送帧 `FA 00 00 LEN DATA F5`，上电一次并在座位状态稳定变化时上报二进制 `10 S1 S2 S3` |
...
| USART1 | ZigBee 测试 | PA9 TX / PA10 RX | USART 异步串口 38400 8N1 | 不适用 | 已初始化 | 是 | 待 FF 80/81/82 实机验证 | CC2530 点对点通信通道，支持 TX/RX 缓冲验证，不跑复杂 ZigBee 业务协议 |
...
| 串口 | 职责 | 当前状态 |
|------|------|----------|
| USART1 | ZigBee 串口点对点通信，38400 | 已初始化，支持 TX/RX 测试，不接业务 |
...
| `FF 82 00 FF` | 通过 USART1 ZigBee 地址帧向 `0x301E` 发送二进制 `01 seq` PING |
| `FF 84 00 FF` | 打印 ZigBee 二进制帧统计 |
| `FF 85 00 FF` | 清空 ZigBee 二进制帧统计 |
...
| `FF B6 00 FF` | 打印本地已注册卡列表 |
| `FF B7 00 FF` | 清空 RAM 卡表 |
